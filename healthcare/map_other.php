...
while($r = mysqli_fetch_array($a1)){
    ...
    $check_id = $l;
     ...
     $querry2 = "SELECT * FROM `medical_donation` where `donation_id`='$check_id';"; 
   ...
   $c1=0;
    ...
    while($r = mysqli_fetch_array($query_ex1))
    {
     ...
     if($map_id==$check_id)
     {
         ...
         $sql1="UPDATE `other_map` SET `name`='$name',`age`='$age' ,`email`='$email1',`user_number`='$user_number',`donation_type`='$don_type',`quantity`='$qty',`country`='$count',`city`='$c',`hospital`='$b',`address`='$ad',`phone_number`='$p',`rating`='$rate',`website`='$w',`user_id`='$id',`map_id`='$check_id' WHERE `map_id` = '$check_id';";
...
$sql="INSERT INTO `other_map`(`name`,`age`,`email`,`user_number`,`donation_type`,`quantity`,`country`, `city`, `hospital`, `address`, `phone_number`,`rating`, `website`, `user_id`,`map_id`) VALUES ('$name','$age','$email1','$user_number','$don_type','$qty','$count','$c','$b','$ad','$p','$rate','$w','$id','$check_id');";
...
//   if(($ex)<=($today)){
//     //  echo $l;
//       $q3="DELETE FROM plasma_donation WHERE expiry_date = '$ex'";
...
<table class="container">
  <thead>
    <tr>
      <th><h1>Title</h1></th>
      <th><h1>Donation Hospital Information</h1></th>
  ...
  <tbody>
    ...
    <tr>
      <td><b>Donation Hospital Name:</b></td>
...
<button class="button button1" onclick="window.history.go(-1)" >Update Donation Hospital Details</button>
<form method="post" action="cancel_other.php" style="margin-top:10px">
